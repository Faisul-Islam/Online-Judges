...
ll traceBack(vl& ar, ll val, ll pos) {
    ll cnt = 0;
    for (int i = pos; i >= 0; i--) {
        if (ar[i] == val) cnt++;
        else break;
...
void solve(ll testCaseChecker)
{
    ...
    F0R(i, n) {
        if (i == 0) {
            ...
            if (ar[i] != cp.fi and ar[i] != cp.se) {
                if (abs(ar[i] - ar[i - 1]) <= 1) {
                    cp = { ar[i - 1] , ar[i] };
                    cLen = traceBack(ar, ar[i - 1], i - 1) + 1;
                }
                else {
                    cp = { ar[i], 0 };
                    cLen = 1;
            ...
            else if (ar[i] != cp.fi and cp.se == 0 and abs(ar[i] - cp.fi) <= 1) {
                cp.se = ar[i];
                cLen++;
                ans = max(ans, cLen);
            }
            ...
            else if (ar[i] == cp.fi or ar[i] == cp.se) {
                cLen++;
                ans = max(ans, cLen);
            }
        }
