...
void pre(){
    F0R(i,61){
     unsigned long long val = powl(2,i);
     powers.pb(to_string(val));
    }
}
...
void solve(ll testCaseChecker)
{
    ...
    F0R(i,sz(powers)){
        string ctp = powers[i];
        ll erase = 0, matched = 0,  matchLeft = 0, eraseLeft = 0;
        F0R(j, sz(s)){
            if(matched == sz(ctp)) break;
            if(s[j] != ctp[matched]){
                erase++;
            }else{
                matched++;
        ...
        matchLeft = sz(ctp) - matched;
        if(erase + matched < sz(s)) eraseLeft = sz(s) - (erase + matched);

        ans = min(ans, erase + matchLeft + eraseLeft);
    }
    cout << ans << endl;
}
