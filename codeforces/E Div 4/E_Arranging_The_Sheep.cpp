...
void solve()
{
    ...
    ll ans = 0;
    if(cnt <= 1 or cnt == n) cout << 0 << endl;
    else{
        ++cnt;
        ll mid = cnt/2, midIdx = 0, tmp = 0;
        F0R(i,n){
          if(s[i] == '*') ++tmp;
             if(tmp == mid){
                midIdx = i;
        ...
        ll leftCnt = midIdx, rightCnt = midIdx+1;

        for(int i = midIdx; i >= 0;i--)
            if(s[i] == '*') ans += leftCnt - i, leftCnt--,s[i] = '*';

        FOR(i,midIdx+1,n)
          if(s[i] == '*') ans += i - rightCnt, rightCnt++, s[i] = '*';
