...
void solve(ll testCaseChecker)
{
    ...
    if (n & 1) {
        ...
        for (int i = 1; i < n - 1;i += 2) {
            ll adjMax = max(ar[i - 1], ar[i + 1]);
            ans += (ar[i] <= adjMax) ? abs(ar[i] - adjMax) + 1 : 0;
        }
        cout << ans << endl;
    }
    else {
        ...
        for (int i = 1;i < n - 1; i++) {
            ll adjMax = max(ar[i - 1], ar[i + 1]);
            if (i & 1) {
                dp1[i] = ((ar[i] <= adjMax) ? abs(ar[i] - adjMax) + 1 : 0);
                if (i != 1) dp1[i] += dp1[i - 2];
            }
            else {
                dp2[i] = ((ar[i] <= adjMax) ? abs(ar[i] - adjMax) + 1 : 0);
                if (i != 2) dp2[i] += dp2[i - 2];
            }
        }
        ...
        for (int i = 1;i < n - 1; i++) {
            if (i & 1) {
                ans = min(ans, dp1[i] + (dp2[n - 2] - dp2[i + 1]));
            }
        }
        ans = min(ans, dp2[n - 2]);
        ans = min(ans, dp1[n - 3]);
